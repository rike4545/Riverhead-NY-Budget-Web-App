...
export default function BudgetAccuracyOutliers({
  overBudget, chronicOverrun, noBudget, recoverablePool, columns,
}: {
  overBudget: Outlier[]
  chronicOverrun: Outlier[]
  noBudget: Outlier[]
  recoverablePool: number
  /** The years the newest Supplement's columns cover. */
  columns: { actual: number; adopted: number; ytd: number; tentative: number }
}) {
  ...
  const TABS: { key: Tab; label: string; blurb: string; rows: Outlier[] }[] = [
    {
      ...
      blurb: `Controllable lines where the ${columns.tentative} Tentative sits more than 30% above what the line has actually been running. Together they hold ${usd(recoverablePool)} more than recent spending supports.`,
      rows: overBudget,
    },
    {
      key: 'chronicOverrun',
      label: `Chronic overruns (${chronicOverrun.length})`,
      blurb: `${columns.actual} spending ran well past the ${columns.adopted} adopted amount — the opposite problem, and a sign the baseline is set too low to be believed.`,
      rows: chronicOverrun,
    },
    {
      key: 'noBudget',
      label: `Spending with no budget (${noBudget.length})`,
      blurb: `Real money went out on lines the ${columns.adopted} budget set to zero. Interfund transfers are excluded, so these are not bookkeeping moves.`,
      rows: noBudget,
    },
  ]
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5, minWidth: 680 }}>
          <thead>
            <tr style={{ textAlign: 'left', color: 'var(--rbl-text-muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.4 }}>
              ...
              <th style={{ padding: '8px 10px', textAlign: 'right' }}>{columns.adopted} budget</th>
              <th style={{ padding: '8px 10px', textAlign: 'right' }}>{columns.tentative} Tentative</th>
            ...
            {rows.map((r) => (
              <tr key={r.account} style={{ borderTop: '1px solid var(--rbl-border-subtle)' }}>
                <td style={{ padding: '9px 10px' }}>
                  ...
                  <div style={{ color: 'var(--rbl-text-muted)', fontSize: 11.5, marginTop: 2 }}>{r.fund} · {r.account}{r.page ? ` · p. ${r.page}` : ''}</div>
                </td>
                <td style={{ padding: '9px 10px', textAlign: 'right', whiteSpace: 'nowrap' }}>{usd(r.actual ?? 0)}</td>
                <td style={{ padding: '9px 10px', textAlign: 'right', whiteSpace: 'nowrap' }}>{usd(r.ytd ?? 0)}</td>
                <td style={{ padding: '9px 10px', textAlign: 'right', whiteSpace: 'nowrap' }}>{usd(r.adopted ?? 0)}</td>
                <td style={{ padding: '9px 10px', textAlign: 'right', whiteSpace: 'nowrap' }}>{usd(r.tentative ?? 0)}</td>
                <td style={{ padding: '9px 10px', textAlign: 'right', fontWeight: 800, color: 'var(--rbl-title)', whiteSpace: 'nowrap' }}>{usd(r.excess)}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={6} style={{ padding: '14px 10px', color: 'var(--rbl-text-muted)' }}>No lines match that filter.</td></tr>
            )}
          </tbody>
        </table>
      </div>
