...
export default function PageShell({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#eef3f8 0,#f7f8f5 42%,#ffffff 100%)', color: '#1f2933', fontFamily: 'Inter, Arial, sans-serif' }}>
      <header style={{ background: 'linear-gradient(135deg,#284a69,#4a7297 62%,#284a69)', color: 'white', borderBottom: '5px solid #c99a2e', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'center', flexWrap: 'wrap', boxShadow: '0 14px 34px rgba(18,56,91,.24)' }}>
        <a href={`${base}/`} style={{ color: 'white', textDecoration: 'none', display: 'flex', gap: 12, alignItems: 'center' }}>
          <span style={{ width: 48, height: 48, borderRadius: 8, display: 'grid', placeItems: 'center', background: '#f8f5ec', color: '#284a69', border: '2px solid #c99a2e', fontWeight: 950 }}>RB</span>
          <span>
      ...
      <section style={{ padding: 30, maxWidth: 1380, margin: '0 auto' }}>
        <div style={{ background: '#ffffff', border: '1px solid #d8e0e7', borderTop: '6px solid #4a7297', borderRadius: 12, padding: 28, boxShadow: '0 14px 34px rgba(31,95,143,.10)', marginBottom: 18 }}>
          ...
          <h1 style={{ fontSize: 42, lineHeight: 1.05, margin: '8px 0', color: '#284a69' }}>{title}</h1>
