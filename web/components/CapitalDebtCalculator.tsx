    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px,1fr) minmax(320px,1.3fr)', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <section style={{ ...card, borderLeft: `6px solid ${cmp.banPremium >= 0 ? 'var(--rbl-danger)' : 'var(--rbl-success)'}` }}>
          ...
          <p style={{ color: 'var(--rbl-text-body)', fontSize: 13.5, lineHeight: 1.55, marginTop: 8, marginBottom: 0 }}>
            {cmp.banPremium >= 0
...
function PathCard({
  ...
  return (
    <div style={{ background: 'var(--rbl-surface-2)', border: '1px solid var(--rbl-border-subtle)', borderRadius: 12, padding: 14 }}>
      <div style={{ fontWeight: 900, color: 'var(--rbl-title)', fontSize: 15 }}>{title}</div>
      <div style={{ color: 'var(--rbl-text-muted)', fontSize: 12, marginBottom: 10 }}>{sub}</div>
      ...
      {breakdown && (
        <div style={{ marginTop: 10, display: 'grid', gap: 4 }}>
          {breakdown.map((b) => (
            <div key={b.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--rbl-text-muted)' }}>
              <span>{b.label}</span>
              <strong style={{ color: 'var(--rbl-text-strong)' }}>{usd(b.value)}</strong>
            </div>
          ))}
        </div>
      )}
...
const slider = { width: '100%', accentColor: 'var(--rbl-series-blue)' } as const
