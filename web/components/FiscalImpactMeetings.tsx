  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <section style={{ ...card, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <a href={meetingHref} style={{ color: 'var(--rbl-accent)', fontWeight: 800, fontSize: 13.5, textDecoration: 'none', whiteSpace: 'nowrap' }} title={`Open the Town Board voting record for ${fmtDate(m.meetingDate)}`}>Open this meeting’s voting record →</a>
      </section>
      ...
      <RecordTrail title="Trace the financial finding" intro="Follow the corrected fiscal-impact read back to the vote, the broader budget record, and the underlying evidence." items={[
        { href: `/meetings/?meeting=${encodeURIComponent(m.slug)}`, label: 'Town Board Votes', text: `Open the voting record for ${fmtDate(m.meetingDate)}.` },
      ...
      <p style={{ color: 'var(--rbl-text-muted)', fontSize: 13, lineHeight: 1.5 }}>Source: {m.source.title}. {m.method} This is an independent read, not the Town’s official position — verify against the agenda packet.</p>
    </div>
  )
}
