  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        <span style={{ fontWeight: 800, color: 'var(--rbl-title)', fontSize: 14.5 }}>Use for capital / tax relief</span>
        <span style={{ fontWeight: 800, color: 'var(--rbl-accent)', fontSize: 14 }}>{usd(drawDown)}</span>
      </div>
      <input
        type="range"
        min={0}
      ...
      <div style={{ background: 'var(--rbl-track)', borderRadius: 999, height: 8, overflow: 'hidden', marginTop: 8 }}>
        <div
          style={{
            ...
            height: '100%',
            background: staysAboveMinimum ? 'var(--rbl-fill-success)' : 'var(--rbl-fill-danger)',
            borderRadius: 999,
          }}
        />
      </div>
      ...
      <p style={{ color: staysAboveMinimum ? 'var(--rbl-success-strong)' : 'var(--rbl-danger)', fontSize: 14, marginTop: 10, fontWeight: 700 }}>
        {staysAboveMinimum
