...
export default function ScenariosPage() {
  return (
    <PageShell title="What-if scenarios" subtitle="Play out choices the Town could make — starting with the big one for next year: can it fund services without blowing past the tax cap? These are illustrations for thinking, not Town policy.">
      ...
      <h2 style={{ color: '#284a69', marginBottom: 6 }}>Balance the 2027 budget under the tax cap</h2>
      ...
      <p style={{ color: '#64748b', fontSize: 13.5, lineHeight: 1.55, margin: '14px 0 24px' }}>
        Prefer the written version? The <a href={`${base}/predict-2027/`} style={{ color: '#4a7297', fontWeight: 700 }}>2027 prediction</a> lays
        out the same levers with real magnitudes, and the <a href={`${base}/tax-cap/`} style={{ color: '#4a7297', fontWeight: 700 }}>Tax Cap page</a> covers
        what a proper override takes if the Board decides the services are worth it.
      </p>

      <h2 style={{ color: '#284a69' }}>Other things the Town could weigh</h2>
      <section style={{ display: 'grid', gap: 14 }}>
        {grounded.map((g) => (
          <article key={g.title} style={{ ...card, borderLeft: '6px solid #4a7297' }}>
            <div style={{ color: '#2563eb', fontWeight: 900, fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.4 }}>{g.kicker}</div>
            <h3 style={{ margin: '4px 0 8px', color: '#284a69' }}>{g.title}</h3>
            <p style={{ color: '#334155', fontSize: 14.5, lineHeight: 1.6, margin: '0 0 10px' }}>{g.body}</p>
            <a href={g.href} style={{ color: '#4a7297', fontWeight: 800, fontSize: 14, textDecoration: 'none' }}>{g.cta} →</a>
          </article>
        ))}
      </section>
    </PageShell>
