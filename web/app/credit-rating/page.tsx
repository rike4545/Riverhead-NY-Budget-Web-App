...
const confidenceBadge = (c: 'VERIFIED' | 'REPORTED') => (
  <span
    style={{
      ...
      padding: '2px 7px',
      borderRadius: 999,
      background: c === 'VERIFIED' ? 'var(--rbl-success-bg)' : 'var(--rbl-warn-bg)',
      color: c === 'VERIFIED' ? 'var(--rbl-success-strong)' : 'var(--rbl-warn)',
    }}
  >
    {c}
  </span>
...
export default function CreditRatingPage() {
  return (
    ...
    >
      ...
      <h2 style={{ color: 'var(--rbl-title)' }}>Riverhead&apos;s rating history</h2>
      <section style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'grid', gap: 14 }}>
          {riverheadRatingHistory.map((e) => (
            <div key={e.date} style={{ borderTop: '1px solid var(--rbl-border-subtle)', paddingTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <strong style={{ color: 'var(--rbl-title)', fontSize: 14.5 }}>{e.date} — {e.action}: {e.rating}</strong>
                {confidenceBadge(e.confidence)}
              </div>
              {e.quote && (
                <p style={{ color: 'var(--rbl-text-body)', fontSize: 14, lineHeight: 1.6, margin: '6px 0 0', fontStyle: 'italic' }}>
                  &ldquo;{e.quote}&rdquo;
                  <span style={{ display: 'block', color: 'var(--rbl-text-faint)', fontSize: 12.5, fontStyle: 'normal', marginTop: 3 }}>— {e.quoteAttribution}</span>
                </p>
              )}
            </div>
          ))}
        </div>
        <p style={{ color: 'var(--rbl-warn)', fontSize: 13, lineHeight: 1.55, marginTop: 14, marginBottom: 0, background: 'var(--rbl-warn-bg)', border: '1px solid var(--rbl-warn-border)', borderRadius: 10, padding: '10px 12px' }}>
          {riverheadRatingGap}
        </p>
      </section>

      <h2 style={{ color: 'var(--rbl-title)' }}>Brookhaven&apos;s AAA</h2>
      <section style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 6 }}>
          <strong style={{ color: 'var(--rbl-title)', fontSize: 14.5 }}>
        ...
        <p style={{ color: 'var(--rbl-text-body)', fontSize: 14, lineHeight: 1.6, margin: '0 0 10px', fontStyle: 'italic' }}>
          &ldquo;{brookhaven.spRationale}&rdquo;
          <span style={{ display: 'block', color: 'var(--rbl-text-faint)', fontSize: 12.5, fontStyle: 'normal', marginTop: 3 }}>
            — S&amp;P rating rationale, as reported {confidenceBadge(brookhaven.spRationaleConfidence)}
          </span>
        </p>
        <p style={{ color: 'var(--rbl-text-strong)', fontSize: 14, lineHeight: 1.6, margin: 0 }}>{brookhaven.history}</p>

        <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--rbl-border-subtle)' }}>
          <strong style={{ color: 'var(--rbl-warn)', fontSize: 13.5 }}>About the quote you sent us</strong>
          <p style={{ color: 'var(--rbl-text-body)', fontSize: 13.5, lineHeight: 1.6, margin: '4px 0 0' }}>
      ...
      <h2 style={{ color: 'var(--rbl-title)' }}>How Riverhead compares to its Suffolk neighbors</h2>
      <section style={{ ...card, marginBottom: 16 }}>
            ...
            .map((t) => {
              ...
              return (
                <div key={t.town} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ width: 130, fontSize: 12.5, color: t.isRiverhead ? 'var(--rbl-warn)' : 'var(--rbl-text-strong)', fontWeight: t.isRiverhead ? 900 : 700 }}>
                    {t.moodyRating ?? '—'}{t.otherAgencyRating ? ` · ${t.otherAgencyRating}` : ''}
                  </span>
                  <span style={{ width: 170, textAlign: 'right', fontSize: 11.5, color: 'var(--rbl-text-faint)' }}>{t.asOf}</span>
                </div>
              )
            })}
        </div>
        <p style={{ color: 'var(--rbl-text-muted)', fontSize: 12.5, marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--rbl-border-subtle)' }}>
      ...
      <h2 style={{ color: 'var(--rbl-title)' }}>What the rating criteria actually weigh</h2>
      <section style={{ ...card, marginBottom: 16 }}>
        <p style={{ color: 'var(--rbl-text-muted)', fontSize: 13.5, marginTop: 0 }}>
        ...
        <div style={{ display: 'grid', gap: 14 }}>
          {ratingCriteria.map((c) => (
            <div key={c.factor} style={{ borderTop: '1px solid var(--rbl-border-subtle)', paddingTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                <strong style={{ color: 'var(--rbl-title)', fontSize: 14.5 }}>{c.factor}</strong>
                <span style={{ color: 'var(--rbl-badge)', fontWeight: 800, fontSize: 13, whiteSpace: 'nowrap' }}>{c.approxWeight}</span>
              </div>
              <p style={{ color: 'var(--rbl-text-muted)', fontSize: 13, margin: '3px 0 6px' }}>{c.whatItMeans}</p>
              <p style={{ color: 'var(--rbl-text-strong)', fontSize: 13.5, lineHeight: 1.55, margin: 0 }}>{c.riverheadRead}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {levers.map((l, i) => (
            <div key={l.title} style={{ display: 'flex', gap: 12 }}>
              <div
                style={{
                  width: 26, height: 26, borderRadius: '50%', background: '#0f766e22', color: 'var(--rbl-teal)',
              ...
              <div>
                <strong style={{ fontSize: 14.5, color: 'var(--rbl-title)' }}>{l.title}</strong>
                <p style={{ color: 'var(--rbl-text-strong)', fontSize: 13.5, lineHeight: 1.6, margin: '4px 0 6px' }}>{l.detail}</p>
                <p style={{ color: 'var(--rbl-text-muted)', fontSize: 12, margin: 0, fontStyle: 'italic' }}>{l.evidence}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ ...card, marginBottom: 16 }}>
        <h3 style={{ marginTop: 0, color: 'var(--rbl-title)' }}>OPEB in context: where Riverhead's per-resident liability ranks</h3>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5, minWidth: 420 }}>
            ...
            <tbody>
              {retireeHealthComparison.towns
                ...
                .sort((a: any, b: any) => b.perResident - a.perResident)
                .map((t: any) => (
                  <tr key={t.name} style={{ borderBottom: '1px solid var(--rbl-border-subtle)', background: t.isRiverhead ? 'var(--rbl-teal-bg)' : undefined, fontWeight: t.isRiverhead ? 800 : 400 }}>
                    <td style={{ padding: '6px 9px', color: t.isRiverhead ? 'var(--rbl-teal)' : 'var(--rbl-title)' }}>{t.name}{t.isRiverhead ? ' ← this site' : ''}</td>
                    <td style={{ padding: '6px 9px', textAlign: 'right', color: t.isRiverhead ? 'var(--rbl-teal)' : 'var(--rbl-text-strong)' }}>{usd(t.perResident)}</td>
                  </tr>
                ))}
            </tbody>
          </table>
      ...
      <section style={{ ...card, marginBottom: 16, borderLeft: '6px solid #9333ea' }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {opebLevers.map((l, i) => (
            <div key={l.title} style={{ display: 'flex', gap: 12, borderTop: i > 0 ? '1px solid var(--rbl-border-subtle)' : undefined, paddingTop: i > 0 ? 14 : 0 }}>
              <div
                style={{
                  width: 26, height: 26, borderRadius: '50%', background: '#9333ea22', color: 'var(--rbl-violet)',
                  display: 'grid', placeItems: 'center', fontSize: 12, fontWeight: 800, flexShrink: 0,
                }}
              >
                {i + 1}
      ...
      <section style={{ ...card, marginBottom: 16 }}>
        <h3 style={{ marginTop: 0, color: 'var(--rbl-title)' }}>Limits of this page</h3>
        <ul style={{ margin: 0, paddingLeft: 0, listStyle: 'none', display: 'grid', gap: 9 }}>
          {caveats.map((c) => (
            <li key={c} style={{ display: 'flex', gap: 9, alignItems: 'baseline', color: 'var(--rbl-text-body)', fontSize: 14, lineHeight: 1.6 }}>
