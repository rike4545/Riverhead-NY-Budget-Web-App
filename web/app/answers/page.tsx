    ...
    >
      <section style={{ background: 'linear-gradient(100deg,#0f2942,var(--rbl-fill-accent))', color: 'white', borderRadius: 16, padding: '20px 22px', marginBottom: 18, boxShadow: '0 14px 34px var(--rbl-shadow)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
          {[
            ['Why did my taxes go up?', '#my-taxes'],
            ['Who gets paid the most?', '#people'],
            ['How much does the Town owe?', '#debt'],
            ['What happens in 2027?', '#next-year'],
          ].map(([label, href]) => (
            <a key={href} href={href} style={{ background: 'rgba(255,255,255,.12)', border: '1px solid rgba(255,255,255,.28)', color: 'white', fontWeight: 800, fontSize: 13.2, padding: '7px 12px', borderRadius: 999, textDecoration: 'none' }}>{label}</a>
          ))}
          <a href={`${base}/search/`} style={{ background: 'var(--rbl-cta-bg)', color: 'var(--rbl-cta-fg)', fontWeight: 900, fontSize: 13.2, padding: '7px 13px', borderRadius: 999, textDecoration: 'none' }}>Search records →</a>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {t.answers.map((a) => (
              <article key={a.q} style={{ ...card, padding: '16px 18px' }}>
                ...
                <a href={a.href} style={{ color: 'var(--rbl-link)', fontWeight: 800, fontSize: 13.8, textDecoration: 'none' }}>{a.cta} →</a>
              </article>
            ))}
          </div>
        </section>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {limits.map((l) => (
            <div key={l.q} style={{ background: 'var(--rbl-surface-2)', border: '1px solid var(--rbl-border-subtle)', borderRadius: 12, padding: '14px 16px' }}>
              ...
              {l.href && <a href={l.href} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--rbl-link)', fontWeight: 800, fontSize: 13.6, textDecoration: 'none' }}>{l.cta}</a>}
            </div>
          ))}
        </div>
      </section>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        <a href={`${base}/explore/`} style={{ background: 'var(--rbl-cta-bg)', color: 'var(--rbl-cta-fg)', fontWeight: 800, padding: '11px 20px', borderRadius: 10, textDecoration: 'none', fontSize: 14 }}>Prefer a guided walkthrough? Take the tour →</a>
        <a href={`${base}/search/`} style={{ background: 'var(--rbl-surface-2)', border: '1px solid var(--rbl-border-subtle)', color: 'var(--rbl-link)', fontWeight: 800, padding: '11px 20px', borderRadius: 10, textDecoration: 'none', fontSize: 14 }}>Looking for something specific? Search everything →</a>
      </div>
    </PageShell>
  )
}
